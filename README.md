...
In relational database, the data is stored in tabular format grouped into rows and columns . A collection of rows is called a table. Each row represents a single record in the table and is made up of one or more columns.
So relational databases are databases that store their data in tables, while SQL is a standard language, which means that it will certainly be supported, no matter how your database is managed. 
